...
const TransactionHistoryPage = () => {
    ...
    useEffect(()=>{
        ...
        setPaginationData({page:1,offset:12});
    },[]);
    const transactionHistoryQuery = useQuery<any,any,TransactionQueryResponseType>({
        queryKey:['transactions','history'],
        queryFn: ()=>transaction_apis.search(queryData,paginationData.page,paginationData.offset),
        select: (res)=> res.data,
        staleTime : 10 * 60 * 100,
        gcTime : 10 * 60 * 100,
    });
    useEffect(()=>{
        dispatch(setBreadcrumb([{label:'Dashboard',link:'/dashboard'},{label:'Transaction History',type:'page'}]))
    },[])
    return (
